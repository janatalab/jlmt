function out_st = metric_readout_v2(data_st, params)
...
% 
% REQUIRES
%   data_st - output from jlmt_proc_series containing data you want to
...
outdims = {'stimulus','stimulus_path','space','proc_series','prev_steps',...
    'tc_local','tc_global','readout_window','timevect','metric','value',...
    'experiment'};
out_st.vars = outdims;
...
stimPaths = data_st.data{dsCols.stimulus_path};
nstims = length(stimPaths);

%% Loop over stims, spaces, time constants, metrics
for istim = 1:nstims
    [currStimPath,stimName] = fileparts(data);
